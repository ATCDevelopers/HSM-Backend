...
import { InvoiceTable } from "../drizzle/schema.js";

export const invoiceService = {
  /**
   * Registers a new billing invoice entry after ensuring numbering uniqueness parameters.
   */
  async createInvoice(data: typeof InvoiceTable.$inferInsert) {
   ...
   */
  async updateInvoice(id: string, data: Partial<typeof InvoiceTable.$inferInsert>) {
    ...
    if (!updated) throw new Error("Failed to modify invoice tracking logs");
    return updated;
  },
  ...
  async deleteInvoice(id: string) {
    ...
    if (!deleted) throw new Error("Failed to drop target record row from database disk arrays");
    return deleted;
  }
};
