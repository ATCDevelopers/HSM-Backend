...
export const medicineRepository = {
   ...
   */
  async create(data: typeof MedicinesTable.$inferInsert) {
  ...
  async search(query: string) {
    return await db
      ...
      .where(
        or(
          like(MedicinesTable.genericName, `%${query}%`),
          like(MedicinesTable.drugName, `%${query}%`)
        )
      );
  },
   ...
   */
  async update(id: string, data: Partial<typeof MedicinesTable.$inferInsert>) {
    const [result] = await db
