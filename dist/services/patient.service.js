import { createPatient, getPatientByNhifCard, getPatientByEmail, } from "../repositories/patient.repository.js";
import { createAddress } from "../repositories/patient.repository.js";
export const registerPatient = async (data) => {
    // Check NHIF
    const existingPatientByNhif = await getPatientByNhifCard(data.patient.nhifCard);
    if (existingPatientByNhif) {
        throw new Error("NHIF card already registered");
    }
    // Check email
    const existingPatientByEmail = await getPatientByEmail(data.patient.email);
...
export const updatePatientAndAddress = async (patientId, patientData, addressData) => {
    if (!patientId) {
    ...
    const updatedPatient = await patientRepository.updatePatientWithAddress(patientId, addressId, // Tunapitisha ID ya anwani iliyopatikana kwenye database
...
export class PatientService {
    ...
    async generatePatientPdf(patientId) {
        const patient = await this.patientRepository.getPatientForPdf(patientId);
        if (!patient) {
    ...
    buildPatientPdf(doc, patient) {
        ...
        doc
            ...
            .font("Helvetica")
            .fillColor("#333333")
            .text(`Full Name: ${patient.firstName} ${patient.middleName ?? ""} ${patient.lastName}`)
        ...
        if (patient.address) {
            ...
            doc
                ...
                .text(`City / District: ${address.city} / ${address.district ?? "N/A"}`)
            ...
            doc
                ...
                .fillColor("#c0392b")
                .text("No primary residential address data found for this patient record.");
        }
    }
}
...
////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////
import { DocumentRepository } from "../repositories/patient.repository.js";
export class DocumentService {
     ...
     */
    async getPatientDocumentsList(patientId) {
...
export class PatientServiceDelete {
    ...
    patientRepository = new PatientRepositoryDelete();
    /**
     * Triggers the soft-delete sequence on a patient record
     */
    async deactivatePatient(patientId, currentUserId) {
     ...
     */
    async searchAllPatients(searchTerm) {
