 ...
 */
const slotLockKey = (doctorId, date, time) => `appointment-slot:${doctorId}:${date.toISOString().slice(0, 10)}:${time}`;
const lockDoctorSlot = async (tx, doctorId, date, time) => {
    await tx.execute(sql `SELECT pg_advisory_xact_lock(hashtextextended(${slotLockKey(doctorId, date, time)}, 0))`);
};
export const getDoctorSchedulesRepository = async (doctorId) => {
    return await db
        .select()
...
export const saveDoctorSchedulesRepository = async (doctorId, schedules, updatedBy) => {
...
export const getDoctorLeavesRepository = async (doctorId, startDate, endDate) => {
...
export const addDoctorLeaveRepository = async (doctorId, leaveInput, createdBy) => {
    return await db.transaction(async (tx) => {
...
export const findAppointmentsByDoctorAndDateRangeRepository = async (doctorId, startDate, endDate) => {
    return await db
        .select()
        .from(AppointmentTable)
        .where(and(eq(AppointmentTable.doctorId, doctorId), gte(AppointmentTable.appointmentDate, startDate), lte(AppointmentTable.appointmentDate, endDate), ne(AppointmentTable.status, 'cancelled'), sql `(${AppointmentTable.deletedAt} IS NULL OR ${AppointmentTable.deletedBy} IS NULL)`));
};
...
export const createAppointmentWithTransaction = async (input, createdBy) => {
    return await db.transaction(async (tx) => {
        ...
        if (doctorId) {
            ...
            const existing = await tx
                .select()
                .from(AppointmentTable)
                .where(and(eq(AppointmentTable.doctorId, doctorId), eq(AppointmentTable.appointmentDate, appointmentDate), eq(AppointmentTable.appointmentTime, appointmentTime), ne(AppointmentTable.status, 'cancelled'), sql `(${AppointmentTable.deletedAt} IS NULL OR ${AppointmentTable.deletedBy} IS NULL)`));
...
export const rescheduleAppointmentRepository = async (id, input, updatedBy) => {
    return await db.transaction(async (tx) => {
        const existing = await tx
            .select()
            .from(AppointmentTable)
        ...
        if (targetDoctorId) {
            ...
            const conflicts = await tx
                .select()
                .from(AppointmentTable)
                .where(and(eq(AppointmentTable.doctorId, targetDoctorId), eq(AppointmentTable.appointmentDate, input.appointmentDate), eq(AppointmentTable.appointmentTime, input.appointmentTime), ne(AppointmentTable.id, id), ne(AppointmentTable.status, 'cancelled'), sql `(${AppointmentTable.deletedAt} IS NULL OR ${AppointmentTable.deletedBy} IS NULL)`));
        ...
        if (targetDoctorId && targetDoctorId !== currentAppointment.doctorId) {
            if (currentAppointment.doctorId) {
                await tx
                    ...
                    .where(and(eq(userAppointment.userId, currentAppointment.doctorId), eq(userAppointment.appointmentId, id)));
            }
            await tx
                .insert(userAppointment)
                .values({
...
export const cancelAppointmentRepository = async (id, reason, cancelledBy) => {
    return await db.transaction(async (tx) => {
        const existing = await tx
            .select()
            .from(AppointmentTable)
...
export const updateAppointmentStatusRepository = async (id, newStatus, updatedBy, reason) => {
    return await db.transaction(async (tx) => {
        const existing = await tx
            .select()
            .from(AppointmentTable)
        ...
        if (!APPOINTMENT_STATUS_TRANSITIONS[currentStatus].includes(newStatus)) {
            ...
            throw new Error(`INVALID_TRANSITION: Cannot move from '${currentStatus}' to '${newStatus}'. ` +
...
export const findUpcomingAppointmentsRepository = async (withinHours = 24) => {
    const now = new Date();
    ...
    return await db
        ...
        .where(and(sql `${appointmentInstant} >= ${now}`, sql `${appointmentInstant} <= ${future}`, ne(AppointmentTable.status, 'cancelled'), ne(AppointmentTable.status, 'completed'), ne(AppointmentTable.status, 'no_show'), isNull(AppointmentTable.deletedAt), sql `(${AppointmentTable.deletedAt} IS NULL OR ${AppointmentTable.deletedBy} IS NULL)`));
};
export const getAppointmentReportsRepository = async (filters) => {
